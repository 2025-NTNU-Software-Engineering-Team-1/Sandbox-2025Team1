...
def main():
    ...
    if parts[0] == "sidecar":
        ...
        if len(parts) < 3:
            debug(
                "Invalid sidecar format, expected: sidecar <hostname> <port>")
        ...
        if port == 6379:
            ...
            debug(
                f"Unknown sidecar port {port}, attempting generic TCP connect")
        ...
        if len(parts) < 4:
            debug(
                "Invalid external format, expected: external (ip|url) <host> <port>"
            )
            print("RESULT: FAIL (bad input)")
            return
