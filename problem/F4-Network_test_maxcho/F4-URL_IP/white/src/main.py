...
def check_target(host, port, expect_success=True, label=""):
    ...
    try:
        ...
        if is_success != expect_success:
            print(
                f"{mark} {host}:{port} -> {status} (expected {expected}){extra}"
            )
        else:
...
if __name__ == "__main__":
    ...
    check_target("208.67.222.222",
                 443,
                 expect_success=False,
                 label="not whitelisted")

    # Other IPs
    check_target("140.82.112.3",
                 443,
                 expect_success=False,
                 label="not whitelisted")
    check_target("31.13.87.36",
                 443,
                 expect_success=False,
                 label="not whitelisted")
    ...
    check_target("www.google.com",
                 22,
                 expect_success=False,
                 label="SSH - non-web port")
    check_target("www.google.com",
                 3306,
                 expect_success=False,
                 label="MySQL - non-web port")
    ...
    check_target("142.250.196.196",
                 443,
                 expect_success=True,
                 label="Google IP - resolved from URL")
    ...
    print(
        "  - Whitelisted URLs (www.google.com): Resolved IPs also whitelisted")
