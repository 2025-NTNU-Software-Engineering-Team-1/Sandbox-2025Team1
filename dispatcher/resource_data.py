...
def copy_teacher_resource_for_case(teacher_res_dir: Path, dest_dir: Path,
                                   task_no: int, case_no: int):
    """
    Copy teacher resource files for specific case into dest_dir, stripping prefix.
    
    DEPRECATED: Use prepare_teacher_for_case() instead.
    """
    if not teacher_res_dir or not teacher_res_dir.exists():
        return []
    prefix = f"{task_no:02d}{case_no:02d}_"
    copied = []
    ...
    for path in teacher_res_dir.rglob("*"):
        ...
        try:
            ...
            copied.append(dest)
            logger().debug(
                "teacher resource copied [task=%s case=%s]: %s -> %s",
                task_no,
                case_no,
                path.name,
                dest,
            ...
            logger().warning(
                "teacher resource copy failed [task=%s case=%s file=%s]: %s",
                task_no,
                case_no,
                path,
                exc,
